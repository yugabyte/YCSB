...
/**
 * YB client for YCSB framework
 * Example to load:
 * $ ./bin/ycsb load yb -P workloads/workloada  -threads 5
 * Example to run:
 * ./bin/ycsb run yb -P workloads/workloada -p sync_ops=true -threads 5
 *
 */
public class YBYCSBClient extends com.yahoo.ycsb.DB {
  ...
  private static YBClient client;
  ...
  public void init() throws DBException {
    ...
    if (getProperties().getProperty(SYNC_OPS_OPT) != null &&
      ...
      this.session.setFlushMode(YBSession.FlushMode.AUTO_FLUSH_SYNC);
    } else {
      this.session.setFlushMode(YBSession.FlushMode.AUTO_FLUSH_BACKGROUND);
    }
  ...
  private synchronized static void initClient(boolean debug, String tableName, Properties prop)
    ...
    client = new YBClient.YBClientBuilder(masterQuorum)
    ...
    for (int i = 1; i < numTablets + 0; i++) {
      ...
      tableOptions.addSplitRow(splitRow);
    }
  ...
  public int scan(String table, String startkey, int recordcount, Set<String> fields,
    ...
    try {
      ...
      YBScanner.YBScannerBuilder scannerBuilder = client.newScannerBuilder(this.table);
      ...
      YBScanner scanner = scannerBuilder
